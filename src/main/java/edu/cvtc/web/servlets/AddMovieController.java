...
public class AddMovieController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	  ...
	  try {
	    ...
	    final String playURL = request.getParameter("playURL");
	    ...
	    if(Strings.isNullOrEmpty(title)
	        ...
	        || Strings.isNullOrEmpty(playURL)
	        || Strings.isNullOrEmpty(imageURL)) {
	      
	      request.setAttribute("message", "You must completely fill out the form to submit.");
	      target = "error.jsp";
	      
	    } else {
	      
	      final int lengthInMinutes = Integer.parseInt(lengthInMinutesString);
	      final Movie movie = new Movie(title, director, lengthInMinutes, imageURL, playURL);
	      
	      final MovieDao movieDao = new MovieDaoImpl();
	      movieDao.insertMovie(movie);
	      
	      request.setAttribute("message", title + " has been added to the database.");
	      target = "success.jsp";
	    ...
	    e.printStackTrace();
	    request.setAttribute("message", "Error: You must enter a valid numeric value for length.");
	    target = "error.jsp";
